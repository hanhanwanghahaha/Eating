	...
	<form method="post" action="/Eating/DingdanAdd?userid=<%=userid.get("id") %>">
		<div class="head">
			<div class="hbox"> 
				...
				<p class="hbox2"><a href="guize.jsp" >规则中心</a><a href="/Eating/uiaddtocar">我的购物车</a><a href="###" >新闻资讯</a> </p>
		...
		<table border="0" cellspacing="0" cellpadding="0" class="innerclass" id="cartable">
            ...
            <tr class="orderitem" data-price="<%=m.get("price")%>" data-proid="<%=m.get("proid")%>">
       		<td style="width: 150px;height: 30px;"><img style="width: 80px;height: 80px;" src="upload/<%=m.get("imgurl")%>"/></td>
       		<td ><%=m.get("cainame")%></td>
       		<td ><%=m.get("price")%></td>
       		<td class="pronumber">    
                    ...
                    <input class="text_box"  name="count"  type="text" value="<%=m.get("count")%>" style="width: 26px;text-align: center;border: 1px solid #EEEEEE;height: 23px;margin: 0;"/>
		...
		<p id="carbuy" class="innerclass"><a href="Index">继续购物</a> <span style="padding-left: 20px;color:#3091f2;font-size:18px;">合计 <span id="sumprice"></span>元 </span> <span id="btnorder">去结算</span></p>
		<div id="receiverinfo" class="innerclass">
				...
				<p>
					<span id="btngotoorder">
						<input value="确认下单" type="submit"  id="dl"
							   style="
						   		    width: 390px;
								    height: 40px;
      ...
      <!--尾部开始-->
		...
		$("#btngotoorder").click(function(){
			...
			if(!$("#saddress2").val().length>0)//判断长度是否大于0，取反之后就数不大于0，即：没有输入内容。
			{
				alert("请输入取餐者详细地址!");
				$("#saddress2").focus();//让一个表单元素获得焦点 
				return false;
			}
		...
		//给所有的加号绑定点击事件
		$(".pronumber input.add").click(function(){
		...
		//给所有的减号绑定点击事件
		$(".pronumber input.min").click(function(){
			//点击加号的时候，把当前加号按钮所在的行的商品数量取出来，加上1  然后再放回去。
		...
		function jisuanjiage()
		{
			...
			$(".orderitem").each(function(index,item){
				 ...
				 var buycount=$(item).find("input[name='count']").val();
